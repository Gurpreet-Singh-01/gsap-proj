...
const Navbar = () => {
    ...
    useEffect(() => {
        if (locate === '/agents' || locate === '/projects') setNavColor("black")
    ...
    return (
        <div className='flex fixed top-0 w-full items-start justify-between z-4'>
            ...
            <div
                onMouseEnter={() => {
                    navbarIconRef.current.style.height = "100%"
                }}
                onMouseLeave={() => {
                    navbarIconRef.current.style.height = "0%"
                }}
                className="h-[5vh] hover:cursor-pointer bg-black relative w-[15vw]">
                <div ref={navbarIconRef} className="bg-[#d3fd50] transition-all absolute top-0 w-full h-0"></div>
                <div className="text-white h-full flex items-center justify-end pr-[2vw] relative">
                    <RiMenu3Line size={25} />
                </div>
            </div>
        </div>
